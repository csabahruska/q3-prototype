...
traceRay :: BSPLevel -> Vec3 -> Vec3 -> Int -> Maybe (Vec3,TraceHit)
traceRay = traceHit TraceRay

traceSphere :: Float -> BSPLevel -> Vec3 -> Vec3 -> Int -> Maybe (Vec3,TraceHit)
traceSphere radius = traceHit $ TraceSphere radius

traceBox :: Vec3 -> Vec3 -> BSPLevel -> Vec3 -> Vec3 -> Int -> Maybe (Vec3,TraceHit)
...
traceHit :: TraceType -> BSPLevel -> Vec3 -> Vec3 -> Int -> Maybe (Vec3,TraceHit)
traceHit traceType bsp inputStart inputEnd mask
  | outputFraction == 1 = Nothing 
  | otherwise = Just (outputEnd,th)
 where
    th@TraceHit{..} = checkNode True traceType bsp inputStart inputEnd 0 0 1 inputStart inputEnd mask
...
checkNode noCull traceType bsp@BSPLevel{..} inputStart inputEnd nodeIndex startFraction endFraction start end mask
  -- leaf
  | nodeIndex < 0 || noCull =
      ...
      in mconcat [ checkBrush traceType bsp inputStart inputEnd brushIndex brush
                 ...
                 , brNumSides > 0
                 , shContentFlags (blShaders ! brShaderNum) .&. mask /= 0
                 ]
  -- node
  | startDistance >= offset && endDistance >= offset = checkNode noCull traceType bsp inputStart inputEnd (fst ndChildren) startFraction endFraction start end mask
  | startDistance < -offset && endDistance < -offset = checkNode noCull traceType bsp inputStart inputEnd (snd ndChildren) startFraction endFraction start end mask
  | otherwise =
      ...
      in checkNode noCull traceType bsp inputStart inputEnd (selectChildren side) startFraction middleFraction1 start middle1 mask `mappend`
         checkNode noCull traceType bsp inputStart inputEnd (selectChildren $ not side) middleFraction2 endFraction middle2 end mask
 where
